...
public class BoardPanel extends JPanel implements PropertyChangeListener {
	...
	/*
	 * Later design change to pure fabrication class for routing also change
	 * event name to be more descriptive
	 */
	...
	public void propertyChange(PropertyChangeEvent evt) {
		...
		if (event.equalsIgnoreCase("UpdateBoardAfterMovingPiece")) {
			...
			updateBoardBeforeMovingPiece((AbstractButton) evt.getOldValue(),
					(MovePieceController) evt.getNewValue());
		}
	}
	...
	public void updateBoardEndOfTimer(int boardSize) {
		for (int row = 0; row < boardSize; ++row) {
			for (int col = 0; col < boardSize; ++col) {
				...
				ActionListener[] listeners = buttons.get(row).get(col)
						.getActionListeners();
	...
	@Requires({"buttonClicked != null", "pieceType != null",
			"validMoves != null"})
	public void updateIconAndButtonStateAfterMovingPiece(
			AbstractButton buttonClicked, PieceType pieceType,
			Set<Cell> validMoves) {
	...
	private void populateCustomPiece(int positionX, int positionY,
		...
		try {
			...
			buttons.get(positionX).get(positionY)
					.setIcon(new ImageIcon(pieceImage));
		} catch (IOException e) {
			e.printStackTrace();
		}
		buttons.get(positionX).get(positionY)
				.setActionCommand(pieceType.toString());
	}
	...
	private void restoreViewForOldPos(Map<String, Integer> oldPos) {
		AbstractButton button = buttons.get(oldPos.get("y"))
				.get(oldPos.get("x"));
		button.setIcon(null);
	...
	private void updateBoardAfterMovingPiece(AbstractButton buttonClicked,
		...
		updateIconAndButtonStateAfterMovingPiece(buttonClicked, pieceType,
				validMoves);
		restoreViewForOldPos(oldPos);
	}

	@Requires({"buttonClicked!=null", "movePieceController!=null"})
	private void updateBoardBeforeMovingPiece(AbstractButton buttonClicked,
		...
		for (Cell moves : validMoves) {
			...
			buttons.get(moves.getY()).get(moves.getX())
					.addActionListener(movePieceController);
		}
	}
