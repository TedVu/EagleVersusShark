...
//    }
    ...
    private boolean isSurrounding(Piece piece, Piece affectedPiece){
        ...
        /* For water cell A2.
         * Can immediately capture any opponent on one cell surrounding if on water cell
         * (opponent and itself on water cell) regardless of any opponent position/ability.
         */
        if(this.onWaterCell){
        ...
        if((pieceX + visibility == affectedPieceX ||
            pieceX == affectedPieceX || affectedPieceX - pieceX == visibility) &&
           (pieceY + visibility == affectedPieceY ||
            pieceY == affectedPieceY || affectedPieceY - pieceY == visibility)) {
            /* IF visibility = 2
             *      THEN check if the affected piece is next to leadership eagle
             */
            return true;
        }
